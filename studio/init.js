...
function MakeDefaultTable(table, filename) {
  if (!filename) {
    filename = `tables/${table.split(".").join("/")}.tab.json`;
  }
  const default1 = {
    name: table,
    action: {
  ...
  return createFile(filename, JOSN.stringify(default1));
}

function MakeDefaultForm(form, filename) {
  if (!filename) {
    filename = `forms/${form.split(".").join("/")}.form.json`;
  }
  const default1 = {
    name: form,
    action: {
  ...
  return createFile(filename, JSON.stringify(default1));
}
/**
 * 初始化表格的配置文件。
 * @param table 表格名称
...
function CreateTable(table) {
  ...
  const exist = MakeDefaultTable(table, filename);
  if (exist == false) {
    console.log("需要生成全配置Table,请再执行一次命令");
    return;
  }
  //如果不存在，需要执行两次，要不然yao.table.Setting无法加载文件
  const setting = Process("yao.table.Setting", table);
...
function CreateForm(form) {
  ...
  const exist = MakeDefaultForm(form, filename);
  if (exist == false) {
  ...
  const setting = Process("yao.form.Setting", form);
